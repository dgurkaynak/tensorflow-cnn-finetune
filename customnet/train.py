...
import datetime
from model import CustomNetModel
...
tf.app.flags.DEFINE_string('train_layers', 'conv1,conv2,fc3,fc4,fc5', 'Finetuning layers, seperated by commas')
...
def main(_):
    ...
    train_dir_name = now.strftime('customnet_%Y%m%d_%H%M%S')
    ...
    # Placeholders
    x = tf.placeholder(tf.float32, [FLAGS.batch_size, 64, 64, 3])
    y = tf.placeholder(tf.float32, [None, FLAGS.num_classes])
    dropout_keep_prob = tf.placeholder(tf.float32)

    # Model
    train_layers = FLAGS.train_layers.split(',')
    model = CustomNetModel(num_classes=FLAGS.num_classes, dropout_keep_prob=dropout_keep_prob)
    loss = model.loss(x, y)
    ...
    train_preprocessor = BatchPreprocessor(dataset_file_path=FLAGS.training_file, num_classes=FLAGS.num_classes,
                                           output_size=[64, 64], horizontal_flip=True, shuffle=True, multi_scale=multi_scale)
    val_preprocessor = BatchPreprocessor(dataset_file_path=FLAGS.val_file, num_classes=FLAGS.num_classes, output_size=[64, 64])
